...
public class ZarzadcaBazy extends SQLiteOpenHelper {
    ...
    public void updateKontakt(Kontakt kontakt){
        SQLiteDatabase db = getWritableDatabase();
        ContentValues wartosci = new ContentValues();
        db.execSQL("UPDATE TELEFONY SET IMIE = '" + kontakt.getImie() +"', NAZWISKO = '" + kontakt.getNazwisko()+"', TELEFON = " +
                "'" + kontakt.getTelefon() + "' WHERE NR = 1");
    }
    ...
    public List<Kontakt> dajPoNazwisku(String nazwisko){
        ...
        //("select nr,imie,nazwisko,telefon from telefony where nazwisko=?	order by nazwisko asc", nazwisko);
        while(kursor.moveToNext()){
            Kontakt kontakt = new Kontakt();
            kontakt.setNr(kursor.getLong(0));
            kontakt.setImie(kursor.getString(1));
